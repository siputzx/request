...
function trimTerminator(str: string): string {
  ...
  for (let i = 0; i < end; i++) {
    ...
    if (c === 0x0a || c === 0x0d || c === 0x00) { end = i; break; }
  }
...
export function parseSetCookieString(setCookieValue: string): Cookie | null {
  if (!isNonEmptyString(setCookieValue)) return null;
  ...
  if (CONTROL_CHAR_RE.test(rawNV)) return null;
  ...
  if (CONTROL_CHAR_RE.test(name)) return null;
  ...
  const cookie: Cookie = { name, value: decodedValue, creationTime: now, lastAccessTime: now };

  if (semiIdx === -1) return cookie;
  ...
  while (pos < len) {
    ...
    switch (rawKey) {
      ...
      case "max-age":
        ...
        break;
      case "domain":
        if (val) cookie.domain = val.toLowerCase().replace(/^\./, "");
        break;
      case "path":
...
export function splitCookiesString(cookiesString: string | string[]): string[] {
  ...
  while (pos < len) {
    ...
    while (pos < len) {
      while (pos < len && cookiesString.charCodeAt(pos) <= 0x20) pos++;
      if (pos >= len) break;
...
export function extractSetCookieHeaders(headers: Headers): string[] {
  if (typeof (headers as unknown as { getSetCookie?: unknown }).getSetCookie === "function") {
